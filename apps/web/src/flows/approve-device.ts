...
export async function approvePendingDevice(device:PendingDevice,pin:string){const profile=await loadProfile();if(!profile)throw new Error('profile_not_found');const current=await loadChatKey(profile.familyChatId,pin);const sealedKeyEnvelope=await sealConversationKey(current.key,fromBase64(device.encryptionPublicKey));await api<void>(`/v1/devices/${device.deviceId}/approve`,{method:'POST',body:JSON.stringify({chatId:profile.familyChatId,keyVersion:current.keyVersion,sealedKeyEnvelope})});}
export async function deviceFingerprint(publicKeyBase64:string){const digest=await crypto.subtle.digest('SHA-256',fromBase64(publicKeyBase64));return Array.from(new Uint8Array(digest).slice(0,6),b=>b.toString(16).padStart(2,'0')).join(':').toUpperCase();}
