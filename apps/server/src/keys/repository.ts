...
export async function listPendingDevices(tx:PoolClient,familyId:string){
  const r=await tx.query<{id:string;member_id:string;device_name:string;display_name:string;encryption_public_key:string;created_at:Date}>(`SELECT d.id,d.member_id,d.device_name,m.display_name,d.encryption_public_key,d.created_at FROM devices d JOIN members m ON m.id=d.member_id WHERE d.family_id=$1 AND d.status='pending_key' ORDER BY d.created_at`,[familyId]);
