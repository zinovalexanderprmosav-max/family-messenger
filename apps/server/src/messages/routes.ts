...
async function authorizeChat(pool:DatabasePool,input:{chatId:string;familyId:string;memberId:string;deviceId:string;keyVersion?:number}){
  const r=await pool.query<{key_version:number|null;rotation_to_key_version:number|null}>(`
    SELECT
      (SELECT max(key_version) FROM conversation_key_versions WHERE chat_id=c.id) key_version,
      (SELECT to_key_version FROM chat_key_rotation_requests WHERE chat_id=c.id AND state='pending') rotation_to_key_version
    FROM chats c
    JOIN chat_members cm ON cm.chat_id=c.id
    WHERE c.id=$1 AND c.family_id=$2 AND cm.member_id=$3
  `,[input.chatId,input.familyId,input.memberId]);
  const row=r.rows[0];
  if(!row) return null;
  if(row.rotation_to_key_version!==null){
    return {allowed:false,currentKeyVersion:row.key_version,rotationRequired:true,rotationToKeyVersion:row.rotation_to_key_version};
  }
  if(input.keyVersion!==undefined&&row.key_version!==input.keyVersion){
    return {allowed:false,currentKeyVersion:row.key_version,rotationRequired:false,rotationToKeyVersion:null};
  }
  return {allowed:true,currentKeyVersion:row.key_version,rotationRequired:false,rotationToKeyVersion:null};
}
...
export async function registerMessageRoutes(app:FastifyInstance,pool:DatabasePool,hub:RealtimeHub){
  app.post<{Params:{chatId:string}}>('/v1/chats/:chatId/messages',async(request,reply)=>{
    ...
    if(!auth)return reply.code(404).send({error:'chat_not_found'});
    if(auth.rotationRequired)return reply.code(409).send({error:'key_rotation_required',toKeyVersion:auth.rotationToKeyVersion});
    if(!auth.allowed)return reply.code(409).send({error:'key_version_mismatch',currentKeyVersion:auth.currentKeyVersion});
  ...
  app.get<{Params:{chatId:string};Querystring:{after?:string;limit?:string}}>('/v1/chats/:chatId/messages',async(request,reply)=>{
    const p=await requireSession(request,pool);
    if(p.deviceStatus!=='active')return reply.code(403).send({error:'device_not_active'});
    const auth=await authorizeChat(pool,{chatId:request.params.chatId,familyId:p.familyId,memberId:p.memberId,deviceId:p.deviceId});
    if(!auth)return reply.code(404).send({error:'chat_not_found'});
    const after=/^\d+$/.test(request.query.after??'0')?request.query.after??'0':'0';
    const parsed=Number.parseInt(request.query.limit??'100',10);
    const limit=Math.max(1,Math.min(100,Number.isFinite(parsed)?parsed:100));
    const tx=await pool.connect();
    try{
      const items=await listMessageEnvelopesAfter(tx,request.params.chatId,after,limit);
      return {items,nextCursor:items.at(-1)?.sequence??after};
    }finally{
      tx.release();
    }
  });
